...
class WhatsAppBackendApiClient:
    def __init__(self, base_url: str, bridge_secret: str) -> None:
        self.base_url = base_url.rstrip("/")
        self.bridge_secret = bridge_secret
        self._verbose_logging = bool(get_whatsapp_settings().DEV_VERBOSE_LOGGING)
    ...
    async def _request(
        ...
        effective_timeout = get_whatsapp_settings().WHATSAPP_DEFAULT_TIMEOUT_SECONDS
    ...
    async def ensure_auth(self, phone_number: str) -> tuple[str, str]:
    ...
    async def chat(self, token: str, message: str, session_id: str | None = None) -> dict[str, Any]:
        ...
        async with asyncio.timeout(get_whatsapp_settings().WHATSAPP_CHAT_TIMEOUT_SECONDS):
