...
- Автоопределение URL: если сообщение содержит ссылку или домен (например `smartcloud.kz`, `https://example.com`), tool planning активируется автоматически.
- Поддерживаются цепочки до 3 шагов в одном сообщении (например: `web_search -> web_fetch -> pdf_create`).
- Каждый шаг цепочки ограничен таймаутом (90 с по умолчанию) — зависший tool не блокирует остальные шаги.
